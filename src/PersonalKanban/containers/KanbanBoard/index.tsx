...
const KanbanBoardContainer: React.FC<KanbanBoardContainerProps> = (props) => {
    ...
    const [justAddedTime, setJustAddedTime] = React.useState<
        JustAddedTimeType[]
    >([])
...
*/
    ...
    const handleCardMove = React.useCallback(
        ({
            ...
            const changedDT: string = new Date()
                .toLocaleString()
                .split(',')
                .join('')
        ...
        [choosedUserId, columns, getRecordIndex, usersState]
    )
    const handleRecordHours = React.useCallback(
        async (idRecord: string, hours: number) => {
            // Обновление вывода только что добавленного времени - для каждый задачи своё,
            //если объект есть- обновим его
            setJustAddedTime((prevState) => {
                const index = prevState.findIndex(
                    (item) => item.idRecord === idRecord
                )
                if (index !== -1) {
                    // Обновляем свойство hours
                    const updatedItem = { ...prevState[index], hours }
                    return [
                    ...
                    return [...prevState, { idRecord, hours }]
                }
            })
            ...
            setJustAddedTime((prevState) => {
                ...
                if (prevState.length === updatedState.length) {
                    updatedState.push({ idRecord, hours })
                }
                return updatedState
            })

            const idTask = Number(idRecord)
            const currentTaskStatus = await OpenProjectService.getTask(
                idTask
            ).then((record): any => record._links.status.title)
            const date = new Date()
   ...
   //         let resultHours = 0
    ...
    return (
        <RecordContext.Provider value={{ handleRecordHours, justAddedTime }}>
            <Toolbar
